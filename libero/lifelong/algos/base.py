...
class Sequential(nn.Module, metaclass=AlgoMeta):
    ...
    def learn_one_task(self, dataset, task_id, benchmark, result_summary):
        ...
        if os.path.exists(model_checkpoint_name):
            target = self.policy.module if isinstance(self.policy, nn.DataParallel) else self.policy
            target.load_state_dict(torch_load_model(model_checkpoint_name)[0])
        else:
            torch_save_model(self.policy, model_checkpoint_name, cfg=self.cfg)
